//Taking the node with less number of edge as the first first seed. Everything else is like Spici V2.
// From v2. Cluster head selection function is changed. The head will be the minimum node from which the sum of distances to all node in that cluster is minimum.
#include<bits/stdc++.h>
using namespace std;
...
double cluster_head_dijakstra(int node){
    priority_queue< pair<double,int> > pq;
    pq.push(mk(0,node));
    double vis[MX4];
    Rep(i,MX4) vis[i]=9876543422.0;
    ...
    while(!pq.empty()){
        ...
        double weight=temp.ff;
        int u= temp.ss;
        //cout<<"u: "<<u<<"  cost: "<<weight<<endl;
        if(ct==50) return 0;
        ct++;
        Rep(i,sz(graph[u])){
            int v=graph[u][i].ff;
            double cost=weight+graph[u][i].ss;
            if(cost>vis[v]) continue;
            if(clusters[node]!=clusters[v]) continue;
...
void controller_head_set(){
    ...
    for(it=mp.begin();it!=mp.end();it++){
        ...
        double haha=98888888881.9;
        int cc;
        For(i,1,n){

            if(clusters[i]==no) {
                //cout<<"Before dijakstra: "<<no<<endl;
                double ans= cluster_head_dijakstra(i);
                //cout<<"after dijakstra: "<<no<<endl;
                if(ans<haha){
                    haha=ans;
                    cc=i;
                }
...
void clusters_head_distance(){
    ...
    for(int i=1;i<=n;i++){
        if(cluster_head[i]>0) st.insert(cluster_head[i]);
    }
    ...
    cout<<"average head to head distance is : "<<love_avg<<endl;
}
void cluster_diameter(){
    ...
    cout<<"Average diameter:  "<<avg_diameter<<endl;
}
int main()
{
    //fwrite
