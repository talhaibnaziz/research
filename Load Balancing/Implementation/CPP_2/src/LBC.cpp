...
LBC::LBC(std::vector<double> a, std::vector<edge> e, int n, double min_bandwidth, int k)
{
    //ctor
...
std::vector<int> LBC::cluster(double dis[MAX_NODE_COUNT][MAX_NODE_COUNT])
{
    int k = cluster_count;
    int n = node_count;
    int node_idx;
...
void LBC::remove_node(int idx, double dis[MAX_NODE_COUNT][MAX_NODE_COUNT])
{
    for(int i=0; i<node_count; i++)
