...
int* Load_Balance::BestFS_dual(node vertices[MAX_NODE_COUNT])
{
    cout<<"BLB Executing..."<<endl;

    double average_load = total_load / (double)c_count;
    double error = 0.0, max_node_dis;
    for(int i=0; i<c_count; i++)
    {
        max_node_dis = find_max_dis(i);
        if(c_loads[i] > average_load)
        {
            error += (c_loads[i] - average_load)*(c_loads[i] - average_load)*max_node_dis;
        }
//        cout<<"Error factor of "<<controllers[i]<<" (C):  "<<(c_loads[i] - average_load)*(c_loads[i] - average_load)<<endl;
//        cout<<"Max distance: "<<max_node_dis<<endl;
//        cout<<"Error of "<<controllers[i]<<" (C):  "<<(c_loads[i] - average_load)*(c_loads[i] - average_load)*max_node_dis<<endl;
    }
        ...
        for(int i=0; i<node_count; i++)
        {
            ...
            for(int j =0; j<adj_node_count; j++)
            {
                ...
                if(current_cluster != next_cluster)
                {
                    ...
                    ///subtracting previous load differences from estimated error
                    max_node_dis = find_max_dis(current_cluster);
                    new_error -= (c_loads[current_cluster] - average_load) * (c_loads[current_cluster] - average_load) * max_node_dis;

                    max_node_dis = find_max_dis(next_cluster);
                    if(c_loads[next_cluster] > average_load)
                        new_error -= (c_loads[next_cluster] - average_load) * (c_loads[next_cluster] - average_load) * max_node_dis;
                    ...
                    max_node_dis = find_max_dis(current_cluster);
                    if(cc_new_load > average_load)
                        new_error += (cc_new_load - average_load) * (cc_new_load - average_load) * max_node_dis;

                    max_node_dis = find_max_dis(next_cluster);
                    if(nc_new_load > average_load)
                        new_error += (nc_new_load - average_load) * (nc_new_load - average_load) * max_node_dis;

                    cluster[u] = current_cluster;
...
double Load_Balance::find_max_dis(int cluster_id)
{
    double max_dis = INT_MIN;

    for(int i=0; i<node_count; i++)
        if(cluster[i] == cluster_id)
            max_dis = max(max_dis, dis[i][ controllers[cluster_id] ]);
