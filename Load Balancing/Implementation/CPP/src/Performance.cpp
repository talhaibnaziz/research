...
{
    net_omega = 0.0;
    ///Calculate average path setup latency of a switch
    for(int i=0; i<node_count; i++)
    {
        delta[i] = 0.0;
        for(int j=0; j<node_count; j++)
    ...
    /**Calculate net omega through controller
    ...
    for(int i=0; i<c_count; i++)
    {
        ...
        ///converting to milliseconds
        tau[i] *= 1000.0;
        cout<<"Tau: "<<tau[i]<<"  Delta: "<<delta[i]<<endl;
    }
    for(int i=0; i<node_count; i++)
    {
        int cluster_id = cluster[i];
        net_omega += (delta[i]+tau[cluster_id])*loads[i];
    }
    return net_omega;
}
