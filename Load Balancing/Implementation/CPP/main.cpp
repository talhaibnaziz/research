/**
Intelligent-SDN system version 1.0
        ...
        while(entry = readdir(pDIR)){
            ...
            if( strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0 )
            {
                ...
                if(strcmp(entry->d_name, "Kdl.txt") == 0)
                {
                    ...
                    ///Cluster the network
                    LBC_3 lbc3(g1.get_avg_distances(), g1.get_edges(), g1.get_node_count(), g1.get_min_bandwidth(), no_of_controllers);
...
//                    omega = performance.calculate_omega(parent, dis, controllers, clusters);
                    ...
                    cout<<performance.calculate_net_omega(parent, dis, controllers, clusters, s_loads)<<endl;
...
//                if(strcmp(entry->d_name, "Abilene.txt") == 0)
                {
                    char file_name[100];
                    strcpy(file_name, "./dataset/");
    //                strcpy(file_name, "./single_datasets/Abvt/");
                    ...
                    ///Initialize graph from input file
                    graph g1(file_name);
                    cout<<g1.get_node_count()<<' '<<g1.get_edges().size()<<endl;
                    ...
                    */
                    ...
                    for(alpha = 0.5; abs(alpha-0.6)>eps; alpha += 0.1)
                    {
                        ...
                        do
                        {
                            ...
                            ///Evaluate flow-setup performance
                            Performance performance(g1.get_node_count(), no_of_controllers);
                            new_omega = performance.calculate_omega(parent, dis, controllers, clusters)*LATENCY_UNIT;
...
//                        cout<<"Controllers: "<<g1.get_node_name(controllers[i])<<endl;
                    cout<<--no_of_controllers<<' '<<new_omega<<endl;
...
//                    Performance performance(g1.get_node_count(), controllers.size());
    //                fout<<controllers.size()<<' '<<performance.calculate_omega(parent, dis, controllers, clusters)<<endl;
