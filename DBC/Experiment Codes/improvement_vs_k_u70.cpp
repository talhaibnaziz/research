...
//    cout<<"THRESHOLD: "<<threshold<<endl;
    ...
    while(idx<nodecnt && cnt<k)
    {
        ...
        for(int i=0; i<cluster_heads.size(); i++)
        {
            ...
            if(dis[nex_node][node]<Td*maxDeg/nex_deg)
            {
...
int main()
{
    ...
    if(unweighted) fout.open("Results/improvement_vs_k_u70.txt");
    else    fout.open("Results/improvement_vs_k_w70.txt");

    int wks[8] = {0,8,7,10,45,41,36,53};
    int uks[8] = {0,3,4,3,4,4,4,4};

    for(int testcase=4; testcase<=4; testcase++)
    {
...
//            cout<<"TESTCASE: "<<testcase<<endl;
            ...
            int limita = 1, limitb = 50*nodecnt/100;
            cout<<limita<<' '<<limitb<<endl;
            double improvement_threshold, improvement;
            double old_latency;
            int old_k;
            for(k=limita; k<=limitb; k++)
            {
                bccp();
                ...
                {
                    improvement = ((old_latency-mlatency)/old_latency)/(double)(k-old_k);
                    fout<<nodecnt<<' '<<k<<' '<<improvement<<endl;
                    old_k = k; old_latency = mlatency;
                }
            }
