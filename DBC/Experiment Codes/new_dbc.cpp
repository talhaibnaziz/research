...
int main()
{
    ...
    if(unweighted) fout.open("Output/our_avg_unweighted_dbc.txt");
    else    fout.open("Output/our_avg_weighted_dbc.txt");
...
//            cout<<"TESTCASE: "<<testcase<<endl;
            ...
            int limita = 2, limitb = 11*nodecnt/100;
            cout<<limita<<' '<<limitb<<endl;
            double improvement_threshold=0.05, improvement;
            double old_latency;
            int old_k;
            ...
            cout<<nodecnt<<' '<<old_k<<' '<<old_latency<<endl;
