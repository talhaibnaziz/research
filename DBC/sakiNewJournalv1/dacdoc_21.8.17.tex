...
\keywords{Resource Allocation, D2D Pairs, Cellular Users, Relax Online Algorithm, LTE}
   ...
   \label{algorthm2}
    ...
    \Comment{An allocation from C(cellular UEs) to D(D2D pairs)}
       \State Newly arrived D2D pairs and unassigned cellular users are initialized as free.  
		...
		\While{$\exists$  free D2D pair $d \in D$ who still has a cellular user $c \in C$ to request to}
