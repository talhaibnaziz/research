...
\corraddr{Md Sakhawat Hossen, sakhawat@iut-dhaka.edu}
...
\keywords{Resource Allocation, D2D Pairs, Cellular Users, Relaxed Online Algorithm}
...
%\noindent
%In this approach at a moment single D2D In event will be handled. No Cellular In event will cause any change in this algorithm. Cellular Out event where the cellular is assigned to any D2D will go through special process. This  Cellular Out event will be assumed as a new D2D In event where the D2D will be the previously assigned D2D. 
%\noindent
   ...
   \label{algorthm2}
    ...
    \Comment{An allocation from C(cellular UEs) to D(D2D pairs)}
       \State Newly arrived D2D pairs and unassigned cellular users are initialized as free.  
		...
		\While{$\exists$  free D2D pair $d$ who still has a cellular user $c$ to request to}
			...
			\Else
				...
				\If{$c$ prefers $d$ to $d'$ and $(c,d)$ satisfies (\ref{eqn:constraintC}),(\ref{eqn:constraintD})}
       ...
       \State Newly arrived D2D pairs and unassigned cellular users are initialized as free.  
		...
		\While{$\exists$  free D2D pair $d$ who still has a cellular user $c$ to request to}
			...
			\Else
				\State some pair $(c, d')$ already exists
				\If{$c$ prefers $d$ to $d'$ and $(c,d)$ satisfies (\ref{eqn:constraintC}),(\ref{eqn:constraintD})}
					\State Calculate the preference list of $d'$.  
                     \If{$S_{c,d}$ + $S_{c',d'}$ - $S_{c'_0}$ $>$ $S_{c,d'}$ }           
...
\begin{figure*}[t]
	{%
		...
		\fbox{
			...
			\begin{subfigure}{0.32\linewidth}
				...
				\caption{Assignment of Optimal Algorithm}
				\label{fig:Hungarian}
			\end{subfigure}
			\begin{subfigure}{0.32\linewidth}
				\vspace{.25cm}
				...
				\caption{Assignment of RORA and CRORA}
	...
	\caption{Assignment strategy (Optimal vs Proposed Algorithms)}
...
\begin{figure*}[t]
	...
	\caption{Trace analysis (Optimal vs Proposed Algorithms) }
...
\noindent
We simulate different scenarios to evaluate the efficiency of our proposed algorithms (RORA and CRORA). We use C++ programming language to build our simulator that supports LTE system. The research problem we consider is an assignment problem which is one of the fundamental combinatorial optimization problems in the branch of optimization. In our simulation, our main objective is to find the assignments of the D2D pairs with the cellular UEs. Based on the assignments, we need calculate SINR, interference and system sumrate from their respective equations. We need to mention that as we do not need to implement the PHY, MAC and network layer to implement our proposed relax online resource allocation algorithms so a networking simulator is not essential for our simulation study. 
\noindent
We use the same simulation parameters as in \cite{lora, zulhasnine, dara} (Table~\ref{Simulation_table}). A single cell network is considered in the simulations. We consider that, the cellular UEs and the D2D pairs are uniformly distributed in the cell area where D2D pairs (D2D transmitters and D2D receivers) are uniformly distributed in a random cluster with a maximum radius of $15$ meters. All the simulation results presented are an average of $20$ different runs for a particular scenario.  
...
$\gamma_{c,target}^{DL}$     & Random                        \\ \hline
$\gamma_{d,target}^{DL} $    & Random                 \\ \hline
\end{tabular}}
\vspace{-0.4cm}
\end{table}
...
%In \ref{fig:fair_perform} our proposed algorithm is compared with optimal,DARA and LORA. On the other hand \ref{fig:restrict_perform} proposed algorithm is only compare with optimal one as there is no existing variant of restricted version. In \ref{fig:sumrate_f}, \ref{fig:sumrate_unfair}  sumrate is compared. Our proposed algorithm situated just after the optimal algorithm. In case of interference it is observed from figure \ref{fig:inter_f} and \ref{fig:inter_unfair} that optimal, LORA and proposed algorithm is very close to each other. In case of the number of changes in Cellular UE figure \ref{fig:noc_f} and \ref{fig:noc_uf} show that our proposed algorithm changes minimum number of assignment. From this observation we can readily says that our proposed algorithm offers a good online algorithm.
...
\begin{figure}[t]
	{ %
		...
		\setlength{\fboxrule}{1.5pt}%
		\centering
		\includegraphics[width=90mm,height=65mm]{Graph/sumRateFair.jpg}
...
\begin{figure}[t]
	{ %
		...
		\setlength{\fboxrule}{1.5pt}%
		\centering
		\includegraphics[width=90mm,height=65mm]{Graph/changesFair.jpg}
...
\begin{figure}[t]
	{ %
		...
		\setlength{\fboxrule}{1.5pt}%
		\centering
		\includegraphics[width=90mm,height=65mm]{Graph/sumRateRestricted.jpg}
...
\begin{figure}[t]
	{ %
		...
		\centering
		%\vspace{-.5cm}
		\includegraphics[width=90mm,height=65mm]{Graph/ChangesRestricted.jpg}
...
%[This is just a place holder copied from CCNC. Need to rewrite]
%\noindent Sharing RBs of an existing cellular network with D2D pairs can increase the capacity of the system. We propose an algorithm which removes nonviable D2D pairs and matches D2D pairs with cellular users that share RBs while maintaining QoS constraints. Our algorithm returns the optimal sumrate, introduces minimum interference in the network satisfying all the constraints. Allowing multiple sharing of D2D pairs and cellular UEs while satisfying QoS constraints can be considered for maximizing the system sumrate. We plan to investigate this issue in the near future.
\smallskip
\noindent
